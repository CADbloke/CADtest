...
Namespace CADtest.Helpers
  ...
  Public Class BlockDefinition
    ...
    Public Shared Function AddEntitiesToBlockDefinition(Of T As Entity)(blockDefinitionObjectId As ObjectId, entities As ICollection(Of T)) As Boolean
      ...
      Using transaction As Transaction = transactionManager.StartTransaction()
        If blockDefinitionObjectId.ObjectClass = (RXObject.GetClass(GetType(BlockTableRecord))) Then
          Dim blockDefinition As BlockTableRecord = DirectCast(transactionManager.GetObject(blockDefinitionObjectId, OpenMode.ForWrite, False), BlockTableRecord)
          If blockDefinition IsNot Nothing Then
            For Each entity As T In entities
              ...
              If text IsNot Nothing Then
                ...
                incrementXY()
              Else
                Dim mText As MText = TryCast(entity, MText)
    ...
    ''' <returns> true if it succeeds, false if it fails. </returns>
    Public Shared Function AddEntityToBlockDefinition(Of T As Entity)(blockDefinitionObjectId As ObjectId, entity As T) As Boolean
      Return AddEntitiesToBlockDefinition(Of T)(blockDefinitionObjectId, New List(Of T)() From { _
        entity _
      })
    End Function
    ...
    Public Shared Function AddNewBlockDefintion(blockDefinitionName As String) As ObjectId
      ...
      Using transaction As Transaction = transactionManager.StartTransaction()
        Dim blockTable As BlockTable = TryCast(transaction.GetObject(database.BlockTableId, OpenMode.ForWrite), BlockTable)
